...
/// Every width/height/depth assignment [item]'s own three lengths can be
/// turned into by hand — every permutation reachable by physically turning
/// it, deduplicated when two lengths happen to match (those permutations are
/// indistinguishable). [depth] is whatever the plan already gave it — real
/// or invented — and stays fixed throughout when the item has no depth of
/// its own, since there is then no real depth to stand it up on.
///
/// Generated by alternately swapping width-with-height and height-with-depth
/// — two moves whose combination reaches all six permutations of three
/// values, the same way turning a box on two different edges in turn
/// eventually shows you every face, not just two of them.
///
/// Ignores [GearItem.rotatable] — that flag only governs what auto-pack may
/// choose on its own; a person turning one placement by hand is a
/// deliberate override of it.
List<(double width, double height, double depth)> turnableOrientationsOf(
  GearItem item,
  double depth,
) {
  if (!item.hasDepth) {
    return [
      (item.width, item.height, depth),
      (item.height, item.width, depth),
    ];
  }

  final start = (item.width, item.height, item.depth!);
  final seen = {start};
  final orientations = [start];
  var current = start;
  for (var step = 0; step < 5; step++) {
    current = step.isEven
        ? (current.$2, current.$1, current.$3)
        : (current.$1, current.$3, current.$2);
    if (seen.add(current)) orientations.add(current);
  }
  return orientations;
}
