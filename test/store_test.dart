...
void main() {
  ...
  group('laying a view on its side', () {
    ...
    test('views start the way round they were designed', () async {
      final plan = await trolley();

      expect(store.planRecordById(plan.id)!.swappedViews, isEmpty);
    });

    test('swapping is remembered per view', () async {
      final plan = await trolley();

      await store.toggleViewSwap(plan.id, 'side');

      expect(store.planRecordById(plan.id)!.swappedViews, {'side'});
      expect(store.planFor(plan.id)!.swappedViews, contains('side'));
    });

    test('swapping again puts it back', () async {
      final plan = await trolley();
    ...
    test('each view is remembered separately', () async {
      final plan = await trolley();

      await store.toggleViewSwap(plan.id, 'side');
      await store.toggleViewSwap(plan.id, 'top');

      expect(store.planRecordById(plan.id)!.swappedViews, {'side', 'top'});
    });

    test('swapping moves nothing', () async {
      final plan = await trolley();
      final box = await store.addItem(
        name: 'box',
        width: 20,
    ...
    test('a swap survives a reload', () async {
      final plan = await trolley();
      await store.toggleViewSwap(plan.id, 'side');

      final reloaded = await reload();

      expect(reloaded.planRecordById(plan.id)!.swappedViews, {'side'});
    });

    test('a duplicated plan keeps how its views are laid out', () async {
      final plan = await trolley();
      await store.toggleViewSwap(plan.id, 'side');
